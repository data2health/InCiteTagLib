...
public class QueueManager {
	...
	public int domainCount() {
		return domains.size() + reloads.size();
	}
	...
	private synchronized boolean reloadQueue() {
		...
		while (!reloads.isEmpty()) {
			...
			try {
				...
				PreparedStatement stagingStmt = conn.prepareStatement(
						"insert into jsoup.staging" + " select url,id from jsoup.document,jsoup.pattern"
								+ " where url ~ pattern" + "   and indexed is null" + "   and domain = ?"
								+ "   and (suffix is null or suffix not in (select suffix from jsoup.suffix))");
				stagingStmt.setString(1, queueDomain.domain);
				stagingStmt.execute();
				stagingStmt.close();

//				PreparedStatement filterStmtorig = conn.prepareStatement("select id,url"
//						+ " from jsoup.document,jsoup.pattern" + " where domain = ?" + "  and url~pattern"
//						+ "  and url!~'/calendar/'" + "  and url!~'/login/'" + "  and url!~'/directory/listing/'"
//						+ "  and url!~'all-in-one-event-calendar'" + "  and indexed is null"
//						+ "  and (suffix is null or suffix not in (select suffix from jsoup.suffix))");
				PreparedStatement filterStmt = conn.prepareStatement("select id,url from jsoup.staging"
						...
						+ " where (domain='*' or domain=?)" + "   and url~pattern)");
